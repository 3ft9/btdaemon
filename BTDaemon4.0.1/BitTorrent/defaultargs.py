...
def get_defaults(ui):
    assert ui in "btdownloadheadless btdownloadcurses btdownloadgui " \
           "btlaunchmany btlaunchmanycurses btmaketorrentgui btdaemon".split()
    ...
    if ui.startswith('btlaunchmany') or ui == 'btdaemon':
        r.extend([
            ('max_uploads', 6,
             "the maximum number of uploads to allow at once. -1 means a (hopefully) reasonable number based on --max_upload_rate. The automatic values are only sensible when running one torrent at once."),
            ('save_in', '',
    ...
    if ui.startswith('btlaunchmany') or ui in ('btmaketorrentgui', 'btdaemon'):
        r.append(
            ('torrent_dir', '',
    ...
    if ui.startswith('btdaemon'):
        r.extend([
            ('display_interval', 0.5,
                'seconds between updates of displayed information'),
            ('parse_dir_interval', 60,
              "how often to rescan the torrent directory, in seconds" ),
            ('paused_torrent_dir', 'paused/',
              "directory used to store paused torrent files" ),
            ('template_dir', 'templates/',
              "template directory" ),
            ('http_listen_port', 8765,
              "which port the http interface listens on" ),
